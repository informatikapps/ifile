...
import { IconTrash, IconRotateClockwise, IconUserCircle, IconArrowUpRight, IconChevronsLeft, IconChevronsRight } from '@tabler/icons-react';
...
export default function Sampah({ auth, files, kategori }) {
    ...
    const truncateString = (url, maxLength = 32) => {
                    ...
                    <div>
                        <div className='flex items-center justify-between pb-4 mb-4 text-sm border-b border-i-yellow-500 h-fit '>
                            <div>
                                ...
                                <p className='text-[12px]'>Pulihkan atau hapus permanen file-file yang sudah dihapus.</p>
                            </div>
                            ...
                            {currentFiles && currentFiles.map((file, index) => (
                                <tbody>
                                    <tr key={index} className={`text-left ${index % 2 === 0 ? 'bg-i-amber-50' : 'bg-white'}`}>
                                        ...
                                        <td className='text-center border-b-[1px] border-i-amber-500'>
                                            <p className='px-2 py-1 rounded-md'>{file.userRole === 'uploader' ? (file.uploader ? file.uploader.username : 'Unknown') : file.waster ? file.waster.username : 'Unknown'}</p>
                                        </td>
                                        <td className='flex border-b-[1px] border-i-amber-500 items-center justify-center gap-2 px-4 py-[8px]'>
                                            ...
                                            <a onClick={() => handleDeletePermanen(file.id)} className={`cursor-pointer px-1 py-1 flex items-center text-center text-white rounded-md hover:text-gray-900  bg-i-orange-500`}>
                                                <IconTrash size={16} />
                                            </a>
                                        </td>
                                    </tr>
