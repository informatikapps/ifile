...
import { IconCopy, IconUserCircle, IconArrowUpRight, IconPencil, IconTrash, IconChevronsLeft, IconChevronsRight, IconCheck } from '@tabler/icons-react';
...
export default function File({ auth, files, kategori }) {
    ...
    const indextOfLastFile = currentPage * filesPerPage;
    const indexOfFirstFile = indextOfLastFile - filesPerPage;
    const currentFiles = filteredFiles.slice(indexOfFirstFile, indextOfLastFile);
    const pageNumbers = [];
    ...
    useEffect(() => {
        ...
        if (selectedCategory !== '') {
            filtered = filtered.filter((file) => file.kategori && file.kategori.id === Number(selectedCategory));
        }
    ...
    const truncateString = (text, maxLength = 28) => {
                ...
                <div className='flex flex-col justify-between w-4/5 px-5 py-10'>
                    <div>
                        <div className='flex items-center justify-between pb-4 mb-4 text-sm border-b border-i-yellow-500 h-fit '>
                            <div>
                                <p className='text-lg font-semibold'>File Manajer</p>
                                <p className='text-[12px]'>Simpan, kelola, dan organisir file-file yang berkaitan dengan Departemen Informatika.</p>
                            </div>
                            ...
                            {currentFiles && currentFiles.map((file, index) => (
                                <tbody key={index}>
                                    <tr key={index} className={`text-left ${index % 2 === 0 ? 'bg-i-amber-50' : 'bg-white'}`}>
                                        ...
                                        <td className='flex items-center justify-center gap-2 px-4 py-[10px] border-b-[1px] border-i-amber-500'>
